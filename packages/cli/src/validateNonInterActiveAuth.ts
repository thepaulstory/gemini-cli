...
export async function validateNonInteractiveAuth(
  ...
  try {
    ...
    if (!effectiveAuthType) {
      const message = `Please set an Auth method in your ${USER_SETTINGS_PATH} or specify one of the following environment variables before running: GEMINI_API_KEY, GOOGLE_GENAI_USE_VERTEXAI, GOOGLE_GENAI_USE_GCA, AI_PROVIDER=openai-compatible with LLM_BASE_URL and LLM_API_KEY`;
      throw new Error(message);
    }
